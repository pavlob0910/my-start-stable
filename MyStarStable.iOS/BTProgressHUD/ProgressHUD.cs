...
namespace BigTed
{
	...
	{
	    public string BasePath = "BTProgressHUD/";
		...
		public UIImage ErrorImage
		{
			get
			{
				return (IsiOS7 ? UIImage.FromBundle(BasePath + "error_7.png") : UIImage.FromBundle(BasePath + "error.png"));
			}
		}
		...
		public UIImage SuccessImage
		{
			get
			{
				return (IsiOS7 ? UIImage.FromBundle(BasePath + "success_7.png") : UIImage.FromBundle(BasePath + "success.png"));
			}
		}
                ...
                foreach (var testWindow in UIApplication.SharedApplication.Windows)
                {
                    if (testWindow.GetClassName() != typeof(UIWindow).Name)
                    {
                        foreach (var possibleKeyboard in testWindow.Subviews)
                        {
                            ...
                            else if (nativeViewName == "UIInputSetContainerView")
                            {
                                UIView ios8Keyboard = possibleKeyboard.Subviews.First(subView => subView.GetClassName() == "UIInputSetHostView");
                                if (ios8Keyboard != null) 
                                {
                                    return (float)ios8Keyboard.Bounds.Size.Height;
                                }
                            }
                        }
                    }
		...
		void PositionHUD(NSNotification notification)
		{
			...
			CGRect orientationFrame = Window.Bounds;
